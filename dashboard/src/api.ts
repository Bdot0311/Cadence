...
export class Api {
  ...
  setPostState(postId: string, state: 'approved' | 'scheduled' | 'killed', scheduledAt?: string | null): Promise<unknown> { return this.request(`/api/posts/${postId}/state`, { method: 'PATCH', body: JSON.stringify({ state, scheduledAt }) }); }
