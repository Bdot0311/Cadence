...
function Setup({ account, api, refresh }: { account: Account | null; api: Api; refresh: () => Promise<void> }) {
  ...
  async function saveVoice() {
    if (detectedPosts.length < 10) { setMessageText(`Detected ${detectedPosts.length} post${detectedPosts.length === 1 ? '' : 's'}. Add --- on its own line between each post; at least 10 are required.`); return; }
    setSavingVoice(true);
    try { await api.saveVoice(detectedPosts, account?.id); setMessageText('Voice profile saved.'); await refresh(); }
  ...
  return <section className="page"><div className="page-heading"><div><p className="eyebrow">CALIBRATION</p><h1>Make it sound like <em>you</em>.</h1><p>Complete these four steps, then add reviewed posts to the Queue.</p></div></div>{messageText && <div className="notice slim">{messageText}</div>}<div className="setup-guide"><div><strong>{completedSteps}/4 setup steps complete</strong><span>{completedSteps === 4 ? 'Setup is complete. Your next action is adding a post in Queue.' : 'Work from top to bottom. A checkmark means the step is saved.'}</span></div><p><b>What runs automatically today:</b> pacing, publishing approved queue items, token checks, safety gates, and decision logging. Automatic idea and draft generation is not enabled yet.</p></div><div className="setup-stack">
